...
const generateDefaultParticipants = () => {
  const names = ["Basti", "Txitxi", "Garci", "Llaku", "Ortzi", "Milikua"];
...
export default function App() {
  ...
  const filteredPlayersList = useMemo(() => {
    ...
    Object.keys(masterPlayerStats).forEach(player => {
      if (!officialNames.includes(player) && activeSelectedPlayers.has(player)) {
        if (player.toLowerCase().includes(playerSearch.toLowerCase())) {
    ...
    return list;
  }, [playerSearch, masterPlayerStats, activeSelectedPlayers]);
          ...
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-[#3a1020] text-white rounded-2xl p-6 shadow-xl relative overflow-hidden">
              ...
              <p className="text-sm text-slate-300 max-w-2xl">
                Clasificación general de la Porra Mundial'2026. Selecciona una jornada específica para ver los puntos obtenidos únicamente en esa etapa.
              </p>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-100 text-slate-600 text-xs uppercase font-extrabold border-b border-slate-200">
                      ...
                      <th className="py-3 px-4 text-center">Privacidad</th>
                    ...
                    {leaderboard.map((p, index) => {
                      ...
                      return (
                        <tr key={p.id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-4 px-4 text-center">
                            {p.isLocked ? (
                              <span className="inline-flex items-center gap-1 text-slate-500 bg-slate-100 px-2 py-0.5 rounded text-xs font-semibold">
                                <Lock size={12} className="text-amber-600" /> Cerrada (Sella)
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded text-xs font-semibold">
                                <Unlock size={12} /> Abierta (Editable)
                              </span>
                            )}
                          </td>
            ...
            <div className="bg-slate-100 p-4 rounded-xl border border-slate-200 text-xs text-slate-600 flex flex-col md:flex-row gap-4 items-center justify-between">
              <div>
                ...
                <span>Ve a la pestaña "Mi Formulario Roster", introduce tu PIN personal (0000 por defecto) y configura tu equipo sin que los demás lo vean.</span>
              </div>
              <button 
                onClick={() => setActiveTab('form')}
          ...
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            {loggedInParticipantId === null ? (
              <div className="max-w-md mx-auto bg-white rounded-2xl shadow-xl border border-slate-200 p-8 space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-500">
                    Introduce tus credenciales para rellenar de forma independiente tu plantilla del torneo. Tu selección no podrá ser visualizada por otros participantes.
                  </p>
                </div>
              ...
              <div className="space-y-6">
                ...
                {/* Confirm & Save Button for Participants (No Locking, just manual Cloud Sincronization Feedback) */}
                {!activeParticipant.isLocked && (
                    ...
                    <div key={p.id} className="p-3 flex items-center justify-between hover:bg-slate-50 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <button 
                          ...
                          className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-2.5 py-1 rounded font-bold"
                        >
                          Editar
                        </button>
                        <button 
        ...
        {isAdminMode && activeTab === 'admin_teams' && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="bg-gradient-to-br from-amber-500 to-amber-600 text-slate-950 p-6 rounded-2xl shadow-lg">
              ...
              <div className="mt-4 bg-amber-400/50 p-3 rounded-xl border border-amber-500/40">
                <label className="text-xs uppercase font-extrabold tracking-wider text-amber-950 block mb-1.5">Jornada Activa de Carga:</label>
                <div className="flex flex-wrap gap-1">
                  {JORNADAS.map(jor => (
                    <button
                      key={jor}
                      onClick={() => setAdminActiveJornada(jor)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all ${adminActiveJornada === jor ? 'bg-[#5d1a33] text-white font-black scale-105' : 'bg-amber-100 hover:bg-amber-200 text-amber-950'}`}
                    >
                      {jor === 'Fase Final' ? 'Fase Final (Semis + Final)' : `Jornada ${jor}`}
                    </button>
                  ))}
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-3">
              ...
              <input 
                ...
                className="flex-1 bg-transparent border-0 p-1 text-sm text-slate-800 focus:outline-none"
              />
              {teamSearch && (
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2 border-t md:border-t-0 md:border-l md:pl-6 border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          ...
                          <label className="col-span-2 flex items-center gap-2 p-2 bg-amber-50 rounded border-amber-200 border cursor-pointer">
                            <input 
                              type="checkbox" 
        ...
        {isAdminMode && activeTab === 'admin_players' && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-3">
              ...
              <input 
                type="text"
                placeholder="Buscar entre los jugadores activos de la porra..."
                value={playerSearch}
                    ...
                    <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex flex-wrap justify-between items-center gap-2">
                      <div>
                        ...
                        <span className="text-xs bg-indigo-100 text-indigo-800 font-extrabold ml-2.5 px-2 py-0.5 rounded-full">
                          {group === 'Otro' ? 'Otro Jugador' : `Grupo ${group.replace('G','')}`}
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 border-t md:border-t-0 md:border-l md:pl-6 border-slate-200">
                        ...
                        <div className="flex flex-col gap-2 text-xs">
                          ...
                          <label className="flex items-center gap-2.5 p-3 bg-amber-50 rounded border border-amber-200 cursor-pointer hover:bg-amber-100/50 transition-colors">
                            ...
                            />
                            <div>
                              <span className="font-extrabold text-amber-900 block font-semibold">Bota de Oro (+10)</span>
                          ...
                          <label className="flex items-center gap-2.5 p-3 bg-yellow-50 rounded border border-yellow-200 cursor-pointer hover:bg-yellow-100/50 transition-colors">
                            <input 
                              ...
                              className="w-4 h-4 rounded text-yellow-600 focus:ring-yellow-500 cursor-pointer"
                            />
                            <div>
                              <span className="font-extrabold text-yellow-950 block font-semibold">Balón de Oro (+20)</span>
